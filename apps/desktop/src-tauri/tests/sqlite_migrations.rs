...
fn fresh_database_reaches_the_current_schema() {
    ...
    assert_eq!(table_columns(&conn, "playlists"), vec!["id", "name"]);
    assert_eq!(
        table_columns(&conn, "playlist_entries"),
        vec!["id", "playlist_id", "track_id", "position"]
    );
    assert_eq!(
        table_columns(&conn, "play_history"),
        vec!["id", "library_id", "track_id", "played_at_unix_seconds"]
    );
}
